...
/* harmony import */ var _common_api__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! ../common/api */ "./src/common/api.ts");
/* harmony import */ var _common_group__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! ../common/group */ "./src/common/group.ts");
/* harmony import */ var _common_export__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(/*! ../common/export */ "./src/common/export.ts");
...
var AppComponent = /** @class */ (function () {
    function AppComponent(apiService, groupService, exportService) {
    ...
    AppComponent.prototype.handleGetCompetitions = function () {
        ...
        this.apiService.getCompetitions().subscribe(function (comps) {
            ...
            _this.competitionsToChooseFrom = comps.map(function (c) { return c['id']; });
        });
    };
    AppComponent.prototype.handleCompetitionSelected = function (competitionId) {
        ...
        this.apiService.getWcif(this.competitionId).subscribe(function (wcif) {
            _this.groupService.wcif = wcif;
            try {
                _this.groupService.processWcif();
                _this.numberOfEvents = _this.groupService.wcif["events"].length;
                _this.numberOfCompetitors = _this.groupService.wcif["persons"].length;
                _this.competitorsToShow = _this.groupService.wcif.persons;
            }
            catch (error) {
                console.error(error);
                alert('An error occured. Check the console.');
                _this.groupService.wcif = null;
                _this.competitionId = null;
            }
        });
    };
    AppComponent.prototype.handleTotalNumberOfTimersSet = function (value) {
        this.groupService.totalNumberOfTimers = Math.min(1, value);
        this.groupService.setEventConfiguration();
    };
    AppComponent.prototype.handleNumberOfStagesSet = function (value, eventId) {
        var event = this.groupService.wcif.events.filter(function (e) { return e.id === eventId; })[0].configuration;
        event.stages = value;
        event.timers = event.totalTimers / event.stages;
    };
    AppComponent.prototype.handleGenerate = function () {
        var _this = this;
        this.groupService.wcif.events.forEach(function (e) {
            _this.groupService.generateGrouping(e.id);
            _this.countCJRSForEvent(e.id);
        });
        this.groupsGenerated = true;
    };
    ;
    AppComponent.prototype.handleGenerateOneEvent = function (eventId) {
        this.groupService.generateGrouping(eventId);
        this.countCJRSForEvent(eventId);
    };
    AppComponent.prototype.handleExport = function (value) {
        this.groupService.sortCompetitorsByName();
        this.readyForExport = value;
    };
    AppComponent.prototype.handleFilterChanged = function (value) {
        var _this = this;
        this.filter = value;
        ...
        this.competitorsToShow = value === '' ? this.groupService.wcif.persons
            : this.groupService.wcif.persons.filter(function (p) { return p.name.toUpperCase().indexOf(_this.filter.toUpperCase()) > -1; });
    };
    AppComponent.prototype.handleSortByEvent = function (event) {
        this.groupService.sortCompetitorsByEvent(event.id);
    };
    AppComponent.prototype.handleBlurEvent = function (eventId) {
        this.countCJRSForEvent(eventId);
    };
    AppComponent.prototype.countCJRSForEvent = function (eventId) {
        var event = this.groupService.wcif.events.filter(function (e) { return e.id === eventId; })[0];
        var configuration = event.configuration;
        var numberOfGroups = configuration.stages * configuration.scrambleGroups;
        event.groupCounters = [];
        var _loop_1 = function (group) {
            var groupCounter = this_1.groupService.wcif.persons
                .filter(function (p) { return p[eventId].group.split(';').indexOf(group.toString()) > -1; }).length + '|';
            groupCounter += this_1.groupService.wcif.persons
                .filter(function (p) { return p[eventId].group.split(';').indexOf('J' + group) > -1; }).length + '|';
            groupCounter += this_1.groupService.wcif.persons
                .filter(function (p) { return p[eventId].group.split(';').indexOf('R' + group) > -1; }).length + '|';
            groupCounter += this_1.groupService.wcif.persons
                .filter(function (p) { return p[eventId].group.split(';').indexOf('S' + group) > -1; }).length;
    ...
    AppComponent = __decorate([
        ...
        __metadata("design:paramtypes", [_common_api__WEBPACK_IMPORTED_MODULE_1__["ApiService"],
            _common_group__WEBPACK_IMPORTED_MODULE_2__["GroupService"],
            _common_export__WEBPACK_IMPORTED_MODULE_3__["ExportService"]])
    ], AppComponent);
    return AppComponent;
}());
...
var ApiService = /** @class */ (function () {
    function ApiService(httpClient) {
        ...
        this.getToken();
        this.headerParams = new _angular_common_http__WEBPACK_IMPORTED_MODULE_1__["HttpHeaders"]();
    ...
    ApiService.prototype.logIn = function () {
        window.location.href = _environments_environment__WEBPACK_IMPORTED_MODULE_2__["environment"].wcaUrl + "/oauth/authorize?client_id=" + _environments_environment__WEBPACK_IMPORTED_MODULE_2__["environment"].wcaAppId + "&redirect_uri=" + _environments_environment__WEBPACK_IMPORTED_MODULE_2__["environment"].appUrl + "&response_type=token&scope=manage_competitions";
    };
    ApiService.prototype.getCompetitions = function () {
        return this.httpClient.get(_environments_environment__WEBPACK_IMPORTED_MODULE_2__["environment"].wcaUrl + "/api/v0/competitions?managed_by_me=true&start=" + new Date().toISOString(), { headers: this.headerParams });
    };
    ApiService.prototype.getWcif = function (competitionId) {
        return this.httpClient.get(_environments_environment__WEBPACK_IMPORTED_MODULE_2__["environment"].wcaUrl + "/api/v0/competitions/" + competitionId + "/wcif", { headers: this.headerParams });
    };
...
/*!*******************************!*\
  !*** ./src/common/classes.ts ***!
  \*******************************/
/*! exports provided: Wcif, EventConfiguration, GeneralConfiguration */
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "Wcif", function() { return Wcif; });
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "EventConfiguration", function() { return EventConfiguration; });
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "GeneralConfiguration", function() { return GeneralConfiguration; });
var Wcif = /** @class */ (function () {
    function Wcif() {
    }
    return Wcif;
}());
...
var GeneralConfiguration = /** @class */ (function () {
    function GeneralConfiguration() {
        this.skipDelegatesAndOrganizers = true;
        this.everyoneCanScrambleAndRun = false;
        this.bordersOnNametags = true;
    }
...
var ExportService = /** @class */ (function () {
    ...
    ExportService.prototype.csvGroupAndTaskAssignments = function (wcif) {
        var csv = 'Name,' + wcif.events.map(function (event) { return event.id; }).join(',') + '\r\n';
        wcif.persons.forEach(function (p) {
            csv += (p.name + ',');
            csv += wcif.events.map(function (event) { return p[event.id].group; }).join(',');
            csv += '\r\n';
        });
        var filename = 'groupAndTaskAssignments-' + wcif.id + '.csv';
        this.downloadFile(csv, filename);
    };
    ExportService.prototype.csvGroups = function (wcif) {
        var csv = 'Name,' + wcif.events.map(function (event) { return event.id; }).join(',') + '\r\n';
        wcif.persons.forEach(function (p) {
            csv += (p.name + ',');
            csv += wcif.events.map(function (event) { return p[event.id].group.split(';')[0]; }).join(',');
            csv += '\r\n';
        });
        var filename = 'groups-' + wcif.id + '.csv';
        this.downloadFile(csv, filename);
    };
    ExportService.prototype.csvEvents = function (wcif) {
        var _this = this;
        var csv = 'event,label,format,limit,cumulative,cutoff,\r\n';
        wcif.events.forEach(function (e) {
            var label = _this.eventNames.filter(function (f) { return f.id === e.id; })[0].label;
            var format = _this.formats.filter(function (f) { return f.id === e.round1.format; })[0].label;
            var limit = !e.round1.timeLimit ? '' : _this.centisToMinutesAndSeconds(e.round1.timeLimit.centiseconds);
            var cumulative = !!e.round1.timeLimit && e.round1.timeLimit.cumulativeRoundIds.length;
            var cutoff = !e.round1.cutoff ? '' : _this.centisToMinutesAndSeconds(e.round1.cutoff.attemptResult);
            csv += (e.id + ',');
            csv += (label + ',');
            csv += (format + ',');
            csv += (limit + ',');
        ...
        var filename = 'events-' + wcif.id + '.csv';
        this.downloadFile(csv, filename);
    };
    ExportService.prototype.centisToMinutesAndSeconds = function (centiseconds) {
        if (!centiseconds) {
            return '';
        }
    ...
    ExportService.prototype.pdfGroupOverview = function (wcif) {
        var document = {
            content: [],
            styles: {},
            defaultStyle: {
        ...
        wcif.events.forEach(function (event) {
            var configuration = event.configuration;
            if (configuration.skip) {
                return;
            }
            var groups = configuration.scrambleGroups * configuration.stages;
            var _loop_1 = function (i) {
                // For every group, collect all competitors, judges, scramblers and runners
                var competitors = wcif.persons.filter(function (p) { return p[event.id].group.split(';')[0] === ('' + i); });
                var judges = wcif.persons.filter(function (p) { return p[event.id].group.indexOf(';') > -1 && p[event.id].group.split(';')[1].indexOf('J' + i) > -1; });
                var scramblers = wcif.persons.filter(function (p) { return p[event.id].group.indexOf(';') > -1 && p[event.id].group.split(';')[1].indexOf('S' + i) > -1; });
                var runners = wcif.persons.filter(function (p) { return p[event.id].group.indexOf(';') > -1 && p[event.id].group.split(';')[1].indexOf('R' + i) > -1; });
                // Write information to pdf
                var groupSummary = event.id + ' - group ' + i + ' has ' + competitors.length + ' competitors, ' + judges.length + ' judges, '
                    + scramblers.length + ' scramblers and ' + runners.length + ' runners\n';
                groupSummary += 'Competitors: ' + competitors.map(function (p) { return p.name; }).join(', ') + '\n';
                groupSummary += 'Judges: ' + judges.map(function (p) { return p.name; }).join(', ') + '\n';
                groupSummary += 'Scramblers: ' + scramblers.map(function (p) { return p.name; }).join(', ') + '\n';
                groupSummary += 'Runners: ' + runners.map(function (p) { return p.name; }).join(', ') + '\n';
                groupSummary += '\n';
        ...
        var filename = 'namesPerGroupOverview-' + wcif.id + '.pdf';
        pdfMake.createPdf(document).download(filename);
    };
    ExportService.prototype.pdfTableGroupAndTaskAssignments = function (wcif) {
        var document = {
        ...
        var filename = 'tableOverview-' + wcif.id + '.pdf';
        pdfMake.createPdf(document).download(filename);
    };
    ExportService.prototype.pdfPersonalSchedules = function (wcif, bordersOnNametags) {
        var _this = this;
        var document = {
            content: [],
            styles: {
        ...
        wcif.persons.forEach(function (p) {
            var nametag = _this.getOneNametagToFill(bordersOnNametags);
            // Set name and country on nametag
            nametag.table.body[0][1].table.body[0][0].text = p.name.split('(')[0]; // Remove local name
            nametag.table.body[0][1].table.body[1][0].text = _this.getCountryName(p.countryIso2);
        ...
        var filename = 'personalSchedules-' + wcif.id + '.pdf';
        pdfMake.createPdf(document).download(filename);
    };
    ExportService.prototype.getOneNametagToFill = function (bordersOnNametags) {
        return {
            style: 'nametag',
            table: {
                widths: [245, 245],
    ...
    ExportService.prototype.csvForCubeComps = function (wcif) {
        var _this = this;
        var csv = 'Status,Name,Country,WCA ID,Birth Date,Gender,' + wcif.events.map(function (event) { return event.id; }).join(',') + ',Email,Guests,IP' + '\r\n';
        wcif.persons.forEach(function (p) {
            csv += ('a,');
            csv += (p.name + ',');
            csv += (_this.getCountryName(p.countryIso2) + ',');
            csv += (p.wcaId + ',');
            csv += (p.birthdate + ',');
            csv += (p.gender + ',');
            csv += wcif.events.map(function (event) { return (p[event.id].competing ? '1' : '0'); }).join(',');
            csv += (p.email + ',');
            csv += (p.guests + ',');
            csv += ('"",');
            csv += '\r\n';
        });
        var filename = 'cubeComps-' + wcif.id + '.csv';
        this.downloadFile(csv, filename);
    };
    ...
    ExportService.prototype.getCountryName = function (countryCode) {
...
/* harmony import */ var _angular_core__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! @angular/core */ "./node_modules/@angular/core/fesm5/core.js");
/* harmony import */ var _classes__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! ./classes */ "./src/common/classes.ts");
var __decorate = (undefined && undefined.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
...
var GroupService = /** @class */ (function () {
    function GroupService() {
        this.totalNumberOfTimers = 16;
        this.configuration = new _classes__WEBPACK_IMPORTED_MODULE_1__["GeneralConfiguration"]();
    }
    GroupService.prototype.generateGrouping = function (eventId) {
        var _this = this;
        var handler = function (e, s) { return _this.staffIsReadyForGrouping(e, s); };
        var file = document.getElementById('staff')['files'][0];
        var staff = null;
        if (file) {
    ...
    GroupService.prototype.staffIsReadyForGrouping = function (eventId, staff) {
        var _this = this;
        // Make some variables
        var event = this.wcif.events.filter(function (e) { return e.id === eventId; })[0];
        var numberOfCompetitors = event.numberOfRegistrations;
        var configuration = event.configuration;
        var numberOfGroups = configuration.stages * configuration.scrambleGroups;
        var tasks = this.createTaskCounter(configuration); // Variable to keep track of assignments for all groups
        if (configuration.skip) {
            return;
        }
        this.shuffleCompetitors();
        // Determine competitors and which of them can scramble, run and/or judge
        var allCompetitors = this.wcif.persons.filter(function (p) { return p[eventId].competing; });
        var potentialScramblers = this.wcif.persons.filter(function (p) { return p[eventId].competing && _this.canScramble(p, staff, eventId); });
        var potentialRunners = this.wcif.persons.filter(function (p) { return p[eventId].competing && _this.canRun(p, staff); });
        ...
        var group = 0; // Group starts counting at 0, so always display as group+1
        var assignedIds = [];
        // 1. Find scramblers, divide them into groups
        potentialScramblers.forEach(function (p) {
            ...
            group = _this.nextGroup(group, numberOfGroups);
        });
        // 2. Find runners, divide them into groups
        potentialRunners.filter(function (p) { return _this.isNotAssigned(p, assignedIds); }).forEach(function (p) {
        ...
        allCompetitors.filter(function (p) { return _this.isNotAssigned(p, assignedIds); }).forEach(function (p) {
            if (_this.canJudge(p) && tasks[group]['J']['max'] > tasks[group]['J']['count']) {
    ...
    GroupService.prototype.nextGroup = function (group, numberOfGroups) {
        return (group + 1) % numberOfGroups; // Go back to 0 on overflow
    };
    GroupService.prototype.isNotAssigned = function (p, assignedIds) {
        return assignedIds.indexOf(p.registrantId) === -1;
    };
    GroupService.prototype.processWcif = function () {
        // TODO count rounds (only one round?)
        for (var _i = 0, _a = this.wcif.events; _i < _a.length; _i++) {
            var e = _a[_i];
            ...
            e.round1 = e.rounds[0];
            e.startTime = '';
            for (var _b = 0, _c = this.wcif.schedule.venues; _b < _c.length; _b++) {
                var v = _c[_b];
                for (var _d = 0, _e = v.rooms; _d < _e.length; _d++) {
                    var r = _e[_d];
                    for (var _f = 0, _g = r.activities; _f < _g.length; _f++) {
                        var a = _g[_f];
                        if (a.activityCode.startsWith(e.id + '-r1') // This is a round 1 of e
                            && (e.startTime === '' || e.startTime > a.startTime)) {
        ...
        var idsToRemove = [];
        for (var _h = 0, _j = this.wcif.persons; _h < _j.length; _h++) {
            var p = _j[_h];
            if (!!p.registration && p.registration.status !== 'accepted') {
                idsToRemove.push(p.registrantId);
                continue;
            }
            for (var _k = 0, _l = this.wcif.events; _k < _l.length; _k++) {
                var e = _l[_k];
        ...
        this.wcif.persons = this.wcif.persons.filter(function (p) { return idsToRemove.indexOf(p.registrantId) === -1; });
        // Set configuration for events
        this.setEventConfiguration();
    };
    GroupService.prototype.setEventConfiguration = function () {
        var defaults = [
        ...
        var _loop_1 = function (e) {
            e.configuration = defaults.filter(function (d) { return d.id === e.id; })[0];
        };
        for (var _i = 0, _a = this.wcif.events; _i < _a.length; _i++) {
            var e = _a[_i];
            _loop_1(e);
    ...
    GroupService.prototype.shuffleCompetitors = function () {
        var i, j, x;
    ...
    GroupService.prototype.sortCompetitorsByName = function () {
    ...
    GroupService.prototype.sortCompetitorsByEvent = function (eventId) {
        this.wcif.persons = this.wcif.persons.sort(function (a, b) {
    ...
    GroupService.prototype.sortEventsByStartTime = function () {
    ...
    GroupService.prototype.createTaskCounter = function (configuration) {
        var numberOfGroups = configuration.stages * configuration.scrambleGroups;
        var tasks = [
...
module.exports = __webpack_require__(/*! D:\AGE\AGE\src\main.ts */"./src/main.ts");
